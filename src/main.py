...
class game:
    ...
    def player_turn(self):
        player_sel = int(input("What box (from 1 - 9) would you like to draw an X in? "))
        if self.grid[player_sel-1] == " ":
            self.grid[player_sel-1] = "X"
            ...
            if opponent_move != None: opponent_move('O')
        ...
        else: print("That space is already filled in!")
        
    def scan_board(self, a):
        #check diagonals
        if itemgetter(0,8)(self.grid) == (a, a) or itemgetter(2,6)(self.grid) == (a, a) and self.grid[4] == " ":
            self.grid[4] = 'O'
            return
